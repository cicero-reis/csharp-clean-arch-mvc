...
namespace CleanArchMVC.Domain.Entities
{
    public sealed class Product : Entity
    {
        ...
        private void ValidateDomain(string name, string description, decimal price, int stock, string image)
        {
            ...
            DomainExceptionValidation.When(description.Length < 5, "Invalid description is too short, minimun 3 charecters.");

            DomainExceptionValidation.When(price < 0, "Invalid price value.");

            DomainExceptionValidation.When(stock < 0, "Invalid stock value.");

            DomainExceptionValidation.When(image?.Length > 250, "Invalid image name, too long, maximun 250 caracters.");
